...
DEFAULT_OUT_PNG = BASE_DIR / "show" / "priority_big_blocks_map_with_values.png"
...
def _big_cell_priority_value(
    node_record: Dict[str, Any],
    priority_map: Dict[str, float],
) -> float:
    if "priority_sum_small_cells" in node_record:
        return float(node_record["priority_sum_small_cells"])
    contained = node_record.get("contained_small_node_ids") or []
    return float(sum(float(priority_map.get(cid, 0.0)) for cid in contained))


def plot_priority_with_big_blocks_and_values(
    small_graph: Dict[str, Any],
    ...
    fig, ax = plt.subplots(figsize=(14, 11), dpi=250)
    ax.set_aspect("equal")
    ...
    for _, bnf in big_nodes.items():
        ...
        cx, cy = bnf.get("centroid_m", [x0 + 0.5 * width, y0 + 0.5 * height])
        val = _big_cell_priority_value(bnf, priority_map=priority_map)
        label = f"{val:.2f}"
        txt = ax.text(
            float(cx),
            float(cy),
        ...
        txt.set_path_effects([pe.withStroke(linewidth=1.8, foreground="white", alpha=0.92)])

    cbar = fig.colorbar(im, ax=ax, fraction=0.034, pad=0.02)
    cbar.set_label("Small-cell priority heatmap (clamped)")
    ...
    ax.set_title(f"Priority map + big-square boundaries + big-square priority labels{title_suffix}")
    ax.text(
        0.01,
        0.01,
        "Each label = sum of small-cell priorities inside the big square",
        transform=ax.transAxes,
        fontsize=9,
        bbox={"boxstyle": "round,pad=0.2", "fc": "white", "ec": "#777777", "alpha": 0.92},
        zorder=10,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot clamped priority map with big-square boundaries and per-big-cell priority values."
    )
    ...
    plot_priority_with_big_blocks_and_values(
